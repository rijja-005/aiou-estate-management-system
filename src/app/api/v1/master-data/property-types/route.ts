...
import { propertyTypeSchema } from '../../../../../server/master-data/schemas';
import { createPropertyType, listPropertyTypes } from '../../../../../server/master-data/service';
...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const result = await listPropertyTypes(query);
    ...
    return NextResponse.json(errorResponse('MASTER_DATA_LIST_FAILED', 'Unable to fetch property types'), { status: 500 });
  }
}
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const parsed = propertyTypeSchema.safeParse(body);

    if (!parsed.success) {
      return NextResponse.json(errorResponse('VALIDATION_ERROR', 'Invalid property type payload', parsed.error.flatten()), {
        status: 400,
      });
    }

    const created = await createPropertyType(parsed.data, {
    ...
    return NextResponse.json(errorResponse('MASTER_DATA_CREATE_FAILED', 'Unable to create property type'), { status: 500 });
  }
}
