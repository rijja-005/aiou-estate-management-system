...
import { roomTypeSchema } from '../../../../../server/master-data/schemas';
import { createRoomType, listRoomTypes } from '../../../../../server/master-data/service';
...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const result = await listRoomTypes(query);
    ...
    return NextResponse.json(errorResponse('MASTER_DATA_LIST_FAILED', 'Unable to fetch room types'), { status: 500 });
  }
}
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(errorResponse('VALIDATION_ERROR', 'Invalid room type payload', parsed.error.flatten()), {
        status: 400,
      });
    }

    const created = await createRoomType(parsed.data, {
    ...
    return NextResponse.json(errorResponse('MASTER_DATA_CREATE_FAILED', 'Unable to create room type'), { status: 500 });
  }
}
