...
import { departmentSchema } from '../../../../../server/master-data/schemas';
import { createDepartment, listDepartments } from '../../../../../server/master-data/service';
...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json(errorResponse('MASTER_DATA_LIST_FAILED', 'Unable to fetch departments'), { status: 500 });
  }
}
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(errorResponse('VALIDATION_ERROR', 'Invalid department payload', parsed.error.flatten()), {
        status: 400,
      });
    }
    ...
    return NextResponse.json(errorResponse('MASTER_DATA_CREATE_FAILED', 'Unable to create department'), { status: 500 });
  }
}
