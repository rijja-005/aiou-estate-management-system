...
import { facilitySchema } from '../../../../../server/master-data/schemas';
import { createFacility, listFacilities } from '../../../../../server/master-data/service';
...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const result = await listFacilities(query);
    ...
    return NextResponse.json(errorResponse('MASTER_DATA_LIST_FAILED', 'Unable to fetch facilities'), { status: 500 });
  }
}
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(errorResponse('VALIDATION_ERROR', 'Invalid facility payload', parsed.error.flatten()), {
        status: 400,
      });
    }
    ...
    return NextResponse.json(errorResponse('MASTER_DATA_CREATE_FAILED', 'Unable to create facility'), { status: 500 });
  }
}
